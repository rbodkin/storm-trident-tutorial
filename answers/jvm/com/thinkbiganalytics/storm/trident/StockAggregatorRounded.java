 ...
 * Outputs: StockStats
 */
public class StockAggregatorRounded implements CombinerAggregator<Map<String, StockStats>> {
    ...
    public Map<String, StockStats> init( TridentTuple tuple ) {
        ...
        final double dollar_price = tuple.getDoubleByField( "dollar_price" );
        assert dollar_price == new Double(Math.round(dollar_price));

        //map.put( symbol, new StockStats( price, shares ) );
        map.put( symbol, new StockStats( dollar_price, shares ) );
        return map;
    }
